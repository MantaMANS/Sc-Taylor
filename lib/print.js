...
export default async function(m, conn = {
    ...
    // Increase log count
    logCount++;
    conn.spinner = conn.spinner ?? {};
    conn.spinner.instance = conn.spinner.instance ?? {};
    conn.spinner.startTime = conn.spinner.startTime ?? {};
    conn.spinner.intervalId = conn.spinner.intervalId ?? {};
    conn.spinner.spinning = conn.spinner.spinning ?? false;

    if (!(m?.text && m?.isCommand && m?.sender && m?.msg)) {
        if (!conn.spinner.spinning) {
            conn.spinner.startTime = new Date();
            conn.spinner.instance = ora({
                text: chalk.bold.magenta(`🌟 Waiting for message... (0s)\n`),
                spinner: 'moon'
            }).start();
            conn.spinner.intervalId = setInterval(() => conn.spinner.instance.text = chalk.bold.magenta(`Waiting for message... (${Math.round((new Date() - conn.spinner.startTime) / 1000)}s)`), 1000);
            conn.spinner.spinning = true;
        }
    } else if (m?.text && m?.isCommand && m?.sender && m?.msg) {
        if (conn.spinner.spinning) {
            conn.spinner.instance.stop();
            clearInterval(conn.spinner.intervalId);
            conn.spinner.instance = {};
            conn.spinner.spinning = false;
        }
    }
