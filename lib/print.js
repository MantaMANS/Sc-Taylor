...
export default async function(m, conn = {
    ...
    if (m?.isCommand && m?.sender && m?.msg && formatType !== 'Unknown') {
        // Output Message Info
        ...
        console.log(`   ${chalk.cyan('- Total Log Messages')}: ${global.logCount}`);
        console.log(chalk.cyan('╰──────────────────────────────────────···\n'));
    ...
    // Increase log count
    global.logCount++;
global.spinner = global.spinner || { instance: {}, startTime: {}, intervalId: {}, spinning: false };
...
if (!(m?.text && m?.isCommand && m?.sender && m?.msg)) {
    if (!global.spinner.spinning) {
        ...
        global.spinner.intervalId = setInterval(() => {
            ...
            global.spinner.instance.text = `${chalk.bold.magenta('Waiting for message...')} ${chalk.cyan(`(${formattedTime})`)}`;
        }, 1000);
