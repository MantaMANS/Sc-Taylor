    ...
    let sock = Object.defineProperties(conn, {
        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                let res, filename
                const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,` [1], 'base64') : /^https?:\/\//.test(PATH) ? (await (res = await getDataBuffer(PATH))) : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
    ...
    */
        ...
        */
        ...
        parseMention: {
             ...
             */
            async value(text = '') {
                const result = (typeof text === 'string' && !Buffer.isBuffer(text) && !(text instanceof Object)) ?
                    [...text.matchAll(/(\d+)@?(@\d+)?/g)]
...
async function getDataBuffer(url, referer = null) {
    try {
        ...
        return response.ok ? Buffer.from(await response.arrayBuffer()) : new Error(`HTTP error! Status: ${response.status}`);
    } catch (error) {
