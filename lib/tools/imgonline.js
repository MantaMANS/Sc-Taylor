...
class ImageProcessor {
    ...
    async processImage(input, endpoint, options) {
        try {
            ...
            const imageLink = $('a[href*="result_img"]').attr('href') || '';
            const finalImageLink = imageLink && imageLink.startsWith('result_img') ?
                `https://www.imgonline.com.ua/${imageLink}` :
                '';
            const resultText = $('div[style="background-color:#CCFFCC;padding:5px;"]').text().trim();
            const hasil = finalImageLink || (resultText ? resultText : 'Tidak ada hasil');
            return hasil;
        } catch (e) {
            console.log('Error', e);
        }
