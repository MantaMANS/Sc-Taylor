...
function stopSpinner(spinnerInfo) {
    if (spinnerInfo) {
        ...
        console.log(chalk.green('All files checked successfully.'));
    ...
    try {
        ...
        if (errorResults.length > 0) {
            ...
            errorResults.forEach((result, index) => {
                console.log(`${index + 1}. ${formatResult(result)}\nError Message: ${chalk.red(result['Error Message'])}`);
            });
        } else {
...
const formatResult = (result) => {
    ...
    return [
        chalk.white(`File Name: ${chalk.yellow(fileName)}`),
        Line !== NOT_APPLICABLE ? chalk.yellow(`Line: ${chalk.yellow(Line)}`) : '',
        Status === 'Success' ? chalk.green(`Status: ${chalk.blue(Status)}`) : chalk.red(`Status: ${chalk.blue(Status)}`),
        chalk.blue(`Size: ${chalk.green(Size)}`),
        chalk.yellow(`Path: ${chalk.blue(Path)}`),
    ].join(' ');
};
